...
public abstract class ValueGenerator<VALUE>
{
    ...
    private static final ValueGenerator<?> NO_OP_GENERATOR = new NoOpGenerator<>(new CustomGuardrailConfig(new HashMap<String, Object>()
    {{
    ...
    public static <VALUE> ValueGenerator<VALUE> getGenerator(String name, CustomGuardrailConfig config)
    {
        ...
        {
            throw new IllegalStateException(format("Unable to find class %s.", className), ex);
        }
        catch (Exception ex)
        {
            throw new IllegalStateException(format("Unable to create instance of generator of class %s.", className), ex);
        }
    }
}