 ...
 * <p>
 * The instances of this validator, when extended in non-abstract classes, are meant to be used as default validators
 * where there is no configuration specified in e.g. cassandra.yaml for custom guardrail validating
 * a specific value of some type.
 */
public class NoOpValidator<T> extends ValueValidator<T>
{
    ...
    @Override
    public Optional<String> shouldWarn(T value)
    {
        return Optional.empty();
    }

    @Override
    public Optional<String> shouldFail(T value)
    {
        return Optional.empty();
    }
