...
public class CustomGuardrail<VALUE> extends Guardrail
{
    ...
    public void guard(VALUE value, ClientState state)
    {
        ...
        Optional<String> failMessage = currentValidator.shouldFail(value);

        if (failMessage.isPresent())
            fail(failMessage.get(), state);
        else
            currentValidator.shouldWarn(value).ifPresent(this::warn);
    }
    ...
    {
        return Collections.unmodifiableMap(validator.get().getParameters());
    }
    ...
    private static <VALUE> ValueValidator<VALUE> getValidator(String name, CustomGuardrailConfig config)
    {
        ...
        {
            throw new IllegalStateException(String.format("Invalid parameters for validator %s: %s", className, config));
        }
        catch (Exception ex)
        {
            throw new IllegalStateException(String.format("Unable to create instance of validator of class %s.", className), ex);
        }
    }
}
